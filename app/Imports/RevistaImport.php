...
use App\Factories\RevistaFactory;
...
class RevistaImport implements ToModel, WithChunkReading, WithHeadingRow, WithEvents, WithStartRow
{
    /**
    ...
    */
    ...
    public function model(array $row): void
    {
        ...
        $medium = $row['medio'] ?? "";
        $isLoadingTheCorrectMedium = $this->validateMedium($medium,strtoupper( TypeOfMedium::Revista->value));

        if ($isLoadingTheCorrectMedium) {
            $this->generalFactory->proccessMedium($row, new RevistaFactory($this->rowNumber), $this->readed_file->id);
        }
