...
class PrensaFactory implements ValidateMediumInterface
{
    ...
    public function validate(array $row, string $readedFile_id): null|Model
    {
        ...
        $publication_date = Carbon::instance(Date::excelToDateTimeObject( $row['direccion']))->format("Y-m-d");
        $mes              = $row['mes'];
        $version          = $row['version'];
        $testigo          = $row['testigo'];

        $uniqueKey = "{$orientacion}-{$pagina}-{$vista_posicion}-{$publication_date}-{$testigo}";
