...
const props = defineProps({
    servicesCategory: Object
});

const form = useForm({
    name: props.servicesCategory.name
});

const submit = () => {
    form.put(route('dashboard.category.update', props.servicesCategory.id));
};
</script>
...
<template>
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl leading-tight">Editar Categorías</h2>
        </template>
