        ...
        .then((response) => {
            if (typeof response.data.data !== undefined) {
                response.data.data.forEach(origin => {
                    const previuslyOrigin = magazines.value.filter(originInArray => originInArray.id === origin.id)
                    if (!previuslyOrigin.length) {
                        magazines.value.push({
                            id: origin.id,
                            name: origin.name,
...
const checkMagazine = (magazine_id) => {
    if (checkedMagazines.length < magazines.value.length) {
        typeAlert.value = 'warning';
        msgAlert.value = '<span>Las marcas seleccionadas <b>no están disponibles</b> en todas las revistas.</span>';
...
const handleExportReport = () => {
    let data = {
        origins_id: checkedMagazines.value,
                ...
                <div v-show="!showTable">
                    ...
                    <div class="my-4">
                        ...
                        <div class="grid grid-cols-4 mt-2 lg:w-8/12 w-full">
                            <div class="p-2" v-if="magazines.length" v-for="magazine in magazines" :key="magazine.id">
                                <div class="border-2 border-gray-200 border-opacity-60 rounded-lg relative flex justify-center overflow-hidden">
                                    <form class="absolute left-0">
                                        <input type="checkbox" class="ml-2" v-model="checkedMagazines" :value="magazine.id"
                                            v-on:change="() => checkMagazine(magazine.id)">
                                    </form>
                                    <img @error="handleErrorImage" class="h-[80px]" :src="magazine.image" alt="Marca" />
                                </div>
                                {{ magazine.name }}
                            </div>
                        </div>
